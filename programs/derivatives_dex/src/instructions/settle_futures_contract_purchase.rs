...
#[derive(Accounts)]
#[instruction(bump_dex_auth: u8, bump_dex_treasury: u8, bump_futures_contract: u8, bump_futures_contract_purchase: u8, bump_payment_token: u8, bump_listed_token: u8)]
pub struct SettleFuturesContractPurchase<'info> {
    ...
    // Destination payment_token account, owned by future_creator
...
pub fn handler(ctx: Context<SettleFuturesContractPurchase>) -> Result<()> {
    ...
    // Ensure futures contract has expired
    if !(now_ts > ctx.accounts.futures_contract.future_expires_ts) {
        return Err(error!(ErrorCode::FuturesContractNotYetExpired));
    }

    // Ensure signer is either creator or purchaser of the futures contract
    ...
    // Do the transfers
    let payment_amount = ctx.accounts.futures_contract_purchase.future_payment_token_amount;
    ...
    token::transfer(ctx.accounts.transfer_future_token_ctx().with_signer(&[&ctx.accounts.derivative_dex.derivative_dex_seeds()]), purchased_amount)?;

    // Either update the future_creator's futures contract or close it (in the case that purchased_amount = listed_amount)
    if purchased_amount != ctx.accounts.futures_contract.future_listed_amount {

        let futures_contract = &mut ctx.accounts.futures_contract;
        futures_contract.future_listed_amount.try_sub_assign(purchased_amount)?;
        futures_contract.future_purchased_amount.try_sub_assign(purchased_amount)?;
